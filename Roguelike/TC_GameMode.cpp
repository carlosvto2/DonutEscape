...
#include "Environment/TC_DungeonGenerator.h"
#include "Shop/TC_Shop.h"
...
void ATC_GameMode::BeginPlay()
{
  ...
  {
    // Create the shop
    ATC_Shop* Shop = NewObject<ATC_Shop>(this, ShopClass);
    if (Shop)
    {
      Shop->Initialize();
      Shop->Destroy();
  ...
  // Continue if it shall create the maze
  if (CreateMaze)
  {
    // call the dungeon generator to create the rooms
    DungeonGenerator = GetWorld()->SpawnActor<ATC_DungeonGenerator>(ATC_DungeonGenerator::StaticClass(),
      FVector::ZeroVector, FRotator::ZeroRotator);
    FVector PlayerStartPosition = DungeonGenerator->GenerateAllDungeonRooms(DungeonSize, MaxRoomSize,
      MinRoomSize, MaxNumberOfRooms, NumberOfRetryPlacingRoom, TileSize);

    // set the player in the position of the first room generated
    if (Donut)
    {
      PlayerStartPosition.Z = 140.f;
      Donut->SetActorLocation(PlayerStartPosition);
    }
  }
...
void ATC_GameMode::SetBoss()
{
  ...
  while(!BossSet)
  {
    ...
    // if it is the initial room, or it has the final door continue
    if (Room == InitialRoom || Room->GetIfHasFinalDoor() || !BossClass || !Room->CheckIfBossCanBeInThisRoom())
      continue;
...
//
//      MagicBox->SetActorScale3D(FVector(1.2f, 1.2f, 3.0f));
//    }
//  }
//}